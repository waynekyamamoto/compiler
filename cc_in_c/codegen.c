...
typedef struct {
    ...
    int *field_is_ptr;      /* NULL if no ptr fields; 1 = pointer field */
} CgStructDef;
...
typedef struct {
    ...
    int is_char_ptr;       /* 1 if char * (scalar char pointer) */
...
static int cg_field_is_ptr(const char *struct_name, const char *field_name) {
    CgStructDef *sd = find_cg_struct(struct_name);
    if (!sd || !sd->field_is_ptr) return 0;
    for (int i = 0; i < sd->nfields; i++) {
        if (strcmp(sd->fields[i], field_name) == 0)
            return sd->field_is_ptr[i];
    }
    return 0;
}
    ...
    if (e->kind == ND_INDEX) {
        ...
        if (e->u.index.base->kind == ND_VAR) {
            ...
            if (is_char_ptr_var(layout, e->u.index.base->u.var_name) ||
                ...
                if (gv && (gv->is_char_array || gv->is_char_ptr)) stride = 1;
            }
            if (stride != 1) {
        ...
        if (stride == 8 && (e->u.index.base->kind == ND_ARROW || e->u.index.base->kind == ND_FIELD)) {
            if (e->u.index.base->u.field.struct_type &&
                ...
                !(cg_field_is_ptr(e->u.index.base->u.field.struct_type,
                                  e->u.index.base->u.field.field) &&
                  is_array_field(e->u.index.base->u.field.struct_type,
                                 e->u.index.base->u.field.field)))
                stride = 1;
        }
        ...
        /* Check if indexing a char pointer or global char array → use byte load */
        {
            ...
            if (e->u.index.base->kind == ND_VAR) {
                ...
                if (is_char_local_array(layout, e->u.index.base->u.var_name))
                    use_byte = 1;
                GlobalVarEntry *gv = find_global(e->u.index.base->u.var_name);
                if (gv && (gv->is_char_array || gv->is_char_ptr))
                    use_byte = 1;
            }
    ...
    if (e->kind == ND_ASSIGN) {
        ...
        /* Check if assigning through a char pointer → use byte store */
        {
            ...
            if (tgt->kind == ND_UNARY && tgt->u.unary.op == '*' &&
                tgt->u.unary.rhs->kind == ND_VAR) {
                if (is_char_ptr_var(layout, tgt->u.unary.rhs->u.var_name))
                    is_char_store = 1;
                else {
                    GlobalVarEntry *gv = find_global(tgt->u.unary.rhs->u.var_name);
                    if (gv && gv->is_char_ptr)
                        is_char_store = 1;
                }
            }
            if (tgt->kind == ND_INDEX &&
                ...
                is_char_store = 1;
            } else if (tgt->kind == ND_INDEX &&
                       tgt->u.index.base->kind == ND_VAR) {
                GlobalVarEntry *gv = find_global(tgt->u.index.base->u.var_name);
                if (gv && (gv->is_char_array || gv->is_char_ptr))
                    is_char_store = 1;
            }
            ...
            if (tgt->kind == ND_INDEX &&
                ...
                !(cg_field_is_ptr(tgt->u.index.base->u.field.struct_type,
                                  tgt->u.index.base->u.field.field) &&
                  is_array_field(tgt->u.index.base->u.field.struct_type,
                                 tgt->u.index.base->u.field.field)))
                is_char_store = 1;
    ...
    if (e->kind == ND_UNARY) {
        ...
        if (op == '*') {
            ...
            if (e->u.unary.rhs->kind == ND_VAR) {
                if (is_char_ptr_var(layout, e->u.unary.rhs->u.var_name))
                    is_char_deref = 1;
                else {
                    GlobalVarEntry *gv = find_global(e->u.unary.rhs->u.var_name);
                    if (gv && gv->is_char_ptr)
                        is_char_deref = 1;
                }
            }
...
char *codegen_generate(Program *prog) {
    ...
    for (int i = 0; i < prog->nstructs; i++) {
        ...
        cg_structs[ncg_structs].field_is_ptr = prog->structs[i].field_is_ptr;
        ncg_structs++;
    }
    ...
    for (int i = 0; i < prog->nglobals; i++) {
        ...
        global_vars[nglobal_vars].is_char_ptr = (gd->is_char && gd->is_ptr && gd->array_size < 0);
