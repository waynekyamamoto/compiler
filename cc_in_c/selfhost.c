...
int *cg_sname[4096];
int **cg_sfields[4096];
int **cg_sfield_types[4096];
int cg_snfields[4096];
int cg_s_is_union[4096];
int *cg_s_bw[4096];     // bit_widths
...
int *glv_stype[16384];
int glv_isptr[16384];
int glv_arrsize[16384];
int glv_is_char[16384];
int nglv;
...
int lay_off[512];
int nlay;
int *lay_arr_name[256];
int lay_arr_count[256];
int lay_arr_inner[256];
int nlay_arr;
int *lay_sv_name[256];
int *lay_sv_type[256];
int nlay_sv;
int *lay_psv_name[256];
int *lay_psv_type[256];
int nlay_psv;
int *lay_unsigned_name[512];
int nlay_unsigned;
...
int nlay_float;
int *lay_barechar_name[512];
int lay_barechar_unsigned[512];
int nlay_barechar;
int lay_stack_size;
...
int *macro_names[16384];
int *macro_values[16384];
int macro_nparams[16384];  // -1 = object-like, >=0 = function-like
...
int macro_ht_next[16384]; // chaining: next macro index in same bucket, -1 = end
int macro_nlens[16384];   // precomputed macro name lengths
...
int *known_funcs[16384];
int nknown_funcs;

// Defined function names (functions with bodies, not just prototypes)
int *defined_funcs[16384];
int ndefined_funcs;

// Bare char parameter tracking (side table indexed by function)
int *barechar_func_names[4096];
int *barechar_param_data[4096];  // array of barechar flags per param
int nbarechar_funcs;

// Variadic function table
int *var_funcs[4096];
int var_nparams[4096];
int nvar_funcs;
...
struct Expr *sl_init_list[4096];
int sl_arr_size[4096];
int *sl_stypes[4096];
int nsl;
...
void merge_string_literals() {
  ...
  int wi = 0;
  while (ri < ntokens) {
    if (tok_kind[ri] == TK_STR && ri + 1 < ntokens && tok_kind[ri + 1] == TK_STR) {
      int *merged = my_strdup(tok_val[ri]);
      while (ri + 1 < ntokens && tok_kind[ri + 1] == TK_STR) {
        ri++;
        merged = build_str2(merged, tok_val[ri]);
      }
      tok_kind[wi] = TK_STR;
      tok_val[wi] = merged;
      tok_pos[wi] = tok_pos[ri];
    } else if (wi != ri) {
      tok_kind[wi] = tok_kind[ri];
      tok_val[wi] = tok_val[ri];
      tok_pos[wi] = tok_pos[ri];
    }
    wi++;
    ri++;
...
int lex(int *src, int srclen) {
  ...
  while (i < len) {
    ...
    if (i + 2 < len) {
      ...
      if (c == '<' && c1 == '<' && c2 == '=') { tok_kind[ntokens] = TK_OP; tok_val[ntokens] = my_strdup("<<="); tok_pos[ntokens] = i; ntokens = ntokens + 1; i = i + 3; continue; }
      if (c == '>' && c1 == '>' && c2 == '=') { tok_kind[ntokens] = TK_OP; tok_val[ntokens] = my_strdup(">>="); tok_pos[ntokens] = i; ntokens = ntokens + 1; i = i + 3; continue; }
    }
...
int *find_lv_stype(int *name) {
  ...
  i = 0;
  while (i < nglv) {
    if (my_strcmp(glv_name[i], name) == 0) {
      return glv_stype[i];
    }
    i++;
  }
...
int set_lv_is_char(int *name) {
  int i = nlv - 1;
  while (i >= 0) {
    if (my_strcmp(lv_name[i], name) == 0) { lv_is_char[i] = 1; return 0; }
    i--;
  }
  return 0;
}
...
int find_lv_is_char(int *name) {
  int i = nlv - 1;
  while (i >= 0) {
    if (my_strcmp(lv_name[i], name) == 0) { return lv_is_char[i]; }
    i--;
  }
  return 0;
}
...
int find_glv_arrsize(int *name) {
  int i = nglv - 1;
  while (i >= 0) {
    if (my_strcmp(glv_name[i], name) == 0) { return glv_arrsize[i]; }
    i--;
  }
  return 0 - 1;
}
...
int *find_glv_stype(int *name) {
  int i = nglv - 1;
  while (i >= 0) {
    if (my_strcmp(glv_name[i], name) == 0) { return glv_stype[i]; }
    i--;
  }
  return 0;
}
...
int find_glv_is_char(int *name) {
  int i = nglv - 1;
  while (i >= 0) {
    if (my_strcmp(glv_name[i], name) == 0) { return glv_is_char[i]; }
    i--;
  }
  return 0;
}
...
int skip_qualifiers() {
  ...
  while (1) {
    if (p_match(TK_KW, "const") || p_match(TK_KW, "volatile") || p_match(TK_KW, "register") || p_match(TK_KW, "restrict") ||
      ...
      cur_pos++;
      skipped = 1;
    } else if (p_match(TK_OP, "[") && cur_pos + 1 < ntokens && my_strcmp(tok_val[cur_pos + 1], "[") == 0) {
      // C23 [[attribute]] syntax - skip [[...]]
      cur_pos += 2; // skip [[
      int bdepth = 1;
      while (cur_pos < ntokens && bdepth > 0) {
        if (p_match(TK_OP, "[") && cur_pos + 1 < ntokens && my_strcmp(tok_val[cur_pos + 1], "[") == 0) { bdepth++; cur_pos += 2; }
        else if (p_match(TK_OP, "]") && cur_pos + 1 < ntokens && my_strcmp(tok_val[cur_pos + 1], "]") == 0) { bdepth--; cur_pos += 2; }
        else { cur_pos++; }
      }
      skipped = 1;
    } else {
...
int td_lookup_is_char(int *name) {
  ...
  while (i >= 0) {
    if (my_strcmp(td_name[i], name) == 0) {
      return td_is_char[i] ? (td_is_unsigned[i] ? 2 : 1) : 0;
    }
    i--;
  }
  return 0;
...
int *parse_base_type() {
    ...
    if (p_match(TK_OP, "{")) {
      ...
      while (!p_match(TK_OP, "}")) {
        ...
        if (p_match(TK_OP, ":")) {
          ...
          struct SFieldInfo *afi2 = my_malloc(48);
          afi2->name = abname;
          afi2->stype = aftype;
          afi2->is_ptr = aptr;
          afi2->bit_width = abw2;
  ...
  if (tok_kind[cur_pos] == TK_ID && !has_typedef(tok_val[cur_pos]) &&
      cur_pos + 1 < ntokens && my_strcmp(tok_val[cur_pos + 1], "(") == 0) {
    return 0;
  }
  printf("Expected type at pos %d tok#%d (got '%s') ntd=%d\n", tok_pos[cur_pos], cur_pos, tok_val[cur_pos], ntd);
...
struct Expr *parse_init_list(int *stype_name) {
  ...
  while (!p_match(TK_OP, "}")) {
    ...
    } else if (tok_kind[cur_pos] == TK_ID && cur_pos + 1 < ntokens && tok_kind[cur_pos + 1] == TK_OP && my_strcmp(tok_val[cur_pos + 1], ":") == 0) {
...
struct Stmt *parse_vardecl_stmt(int vd_is_static) {
  ...
  if (vd_is_static == 0 && p_match(TK_KW, "const") && cur_pos + 1 < ntokens && my_strcmp(tok_val[cur_pos + 1], "static") == 0) {
    vd_is_static = 1;
  }
...
struct Expr *parse_unary() {
    ...
    if (p_match(TK_OP, "(")) {
      ...
      if (tok_kind[cur_pos] == TK_KW && is_type_keyword(tok_val[cur_pos])) {
        ...
        if (tok_kind[cur_pos] == TK_OP && my_strcmp(tok_val[cur_pos], "*") == 0 &&
            cur_pos + 1 < ntokens && tok_kind[cur_pos + 1] == TK_ID) {
          int *deref_name = tok_val[cur_pos + 1];
  ...
  if (p_match(TK_OP, "(")) {
    ...
    if (is_kw_cast || is_td_cast) {
      ...
      if (is_td_cast) {
        ...
        while (ct_scan < ntokens && tok_kind[ct_scan] == TK_KW) {
          if (my_strcmp(tok_val[ct_scan], "unsigned") == 0) { ct_is_unsigned = 1; }
          if (my_strcmp(tok_val[ct_scan], "short") == 0) { ct_is_short = 1; }
          if (my_strcmp(tok_val[ct_scan], "char") == 0) { ct_is_char = 1; }
          ct_scan = ct_scan + 1;
        }
        ...
        {
          int ct_p = ct_scan;
          while (ct_p < ntokens && !(tok_kind[ct_p] == TK_OP && my_strcmp(tok_val[ct_p], ")") == 0)) {
            if (tok_kind[ct_p] == TK_OP && my_strcmp(tok_val[ct_p], "*") == 0) { ct_is_ptr = 1; }
            ct_p = ct_p + 1;
          }
        }
  ...
  if (e != 0) {
    ...
    p_eat(TK_OP, "+");
    return parse_unary();
  } else if (p_match(TK_OP, "&&") && tok_kind[cur_pos + 1] == TK_ID) {
...
struct Expr *parse_primary() {
  ...
  if (k == TK_NUM) {
    ...
    if (my_strcmp(name, "__builtin_offsetof") == 0) {
      ...
      int *ofs_stype = 0;
      if (tok_kind[cur_pos] == TK_ID) { ofs_stype = my_strdup(tok_val[cur_pos]); cur_pos++; }
      p_eat(TK_OP, ",");
    ...
    printf("Unexpected token '%s' at %d tok#=%d k=%d (cur_pos=%d, prev_tok=%s)\n", v, tok_pos[cur_pos], cur_pos, tok_kind[cur_pos], cur_pos, cur_pos > 0 ? tok_val[cur_pos - 1] : "?");
  ...
  while (p_match(TK_OP, "[") || p_match(TK_OP, ".") || p_match(TK_OP, "->") ||
    ...
    if (p_match(TK_OP, "[")) {
      ...
      if (e->kind == ND_NUM && tok_kind[cur_pos] == TK_NUM) {
        cur_pos++; // skip decimal part
        // Skip exponent part if present (e.g., 1.0e10)
        if (tok_kind[cur_pos] == TK_ID && (__read_byte(tok_val[cur_pos], 0) == 'e' || __read_byte(tok_val[cur_pos], 0) == 'E')) {
          cur_pos++;
        }
...
struct Stmt *parse_stmt() {
  ...
  if (p_match(TK_KW, "typedef")) {
    ...
    int *local_td_name = 0;
    int ti = cur_pos;
    while (ti < ntokens && !(tok_kind[ti] == TK_OP && my_strcmp(tok_val[ti], ";") == 0)) {
      if (tok_kind[ti] == TK_OP && my_strcmp(tok_val[ti], "(") == 0 &&
          ti + 1 < ntokens && tok_kind[ti+1] == TK_OP && my_strcmp(tok_val[ti+1], "*") == 0 &&
          ti + 2 < ntokens && tok_kind[ti+2] == TK_ID) {
        local_td_name = tok_val[ti+2];
      }
      ti++;
    }
    if (local_td_name == 0) {
      ti = cur_pos;
      while (ti < ntokens && !(tok_kind[ti] == TK_OP && my_strcmp(tok_val[ti], ";") == 0)) {
        if (tok_kind[ti] == TK_ID) { local_td_name = tok_val[ti]; }
        ti++;
      }
    }
  ...
  if (p_match(TK_KW, "int") || p_match(TK_KW, "char") || p_match(TK_KW, "void") ||
    ...
    skip_qualifiers();
    if (tok_kind[cur_pos] == TK_ID && cur_pos + 1 < ntokens && tok_kind[cur_pos + 1] == TK_OP && my_strcmp(tok_val[cur_pos + 1], "(") == 0) {
...
struct SDef *parse_struct_or_union_def(int is_union) {
  ...
  while (!p_match(TK_OP, "}")) {
    ...
    int *inline_sname = 0;
    if ((p_match(TK_KW, "struct") || p_match(TK_KW, "union")) &&
        ((cur_pos + 2 < ntokens && tok_kind[cur_pos + 1] == TK_ID &&
          tok_kind[cur_pos + 2] == TK_OP && my_strcmp(tok_val[cur_pos + 2], "{") == 0) ||
         (cur_pos + 1 < ntokens && tok_kind[cur_pos + 1] == TK_OP && my_strcmp(tok_val[cur_pos + 1], "{") == 0))) {
      ...
      if (is_anon_member) {
        ...
        if (inner_sdi != 0) {
          ...
          while (ifi < inner_sdi->nflds) {
            ...
            struct SFieldInfo *iff = my_malloc(48);
            iff->name = my_strdup(inner_sdi->flds[ifi]->name);
            iff->stype = inner_sdi->flds[ifi]->stype != 0 ? my_strdup(inner_sdi->flds[ifi]->stype) : 0;
            iff->is_ptr = inner_sdi->flds[ifi]->is_ptr;
            iff->bit_width = inner_sdi->flds[ifi]->bit_width;
            iff->is_array = inner_sdi->flds[ifi]->is_array;
            iff->is_char = inner_sdi->flds[ifi]->is_char;
            finfo[nf] = iff;
            nf++;
            ifi++;
          }
    ...
    if (p_match(TK_OP, ":")) {
      ...
      fields[nf] = aname;
      struct SFieldInfo *afi = my_malloc(48);
      afi->name = my_strdup(aname);
      if (ftype != 0) { afi->stype = my_strdup(ftype); } else { afi->stype = 0; }
      afi->is_ptr = is_ptr;
      afi->bit_width = abw;
      afi->is_array = 0;
      afi->is_char = 0;
  ...
  if (!p_match(TK_OP, ")")) {
    while (1) {
      ...
      if (p_match(TK_ID, 0)) {
        ...
        param_is_intptr[np] = ((is_ptr > 0 || is_funcptr > 0) && p_is_char == 0) ? 1 : 0;
...
struct GDecl *parse_global_decl_one(int *stype, int g_is_char) {
  ...
  if (stype != 0) {
    ...
    glv_arrsize[nglv] = 0 - 1; // updated later when array_size is known
    glv_is_char[nglv] = 0;
    nglv++;
  }
  int array_size = 0 - 1;
  ...
  if (stype == 0 && nglv < 16384) {
    glv_name[nglv] = my_strdup(name);
    glv_stype[nglv] = 0;
    glv_isptr[nglv] = is_ptr;
    glv_arrsize[nglv] = array_size;
    glv_is_char[nglv] = g_is_char;
    nglv++;
  } else if (stype != 0) {
    // Update existing glv entry with array size (it was registered earlier without it)
    int gi = nglv - 1;
    while (gi >= 0) {
      if (my_strcmp(glv_name[gi], name) == 0) { glv_arrsize[gi] = array_size; glv_is_char[gi] = g_is_char; gi = 0 - 1; }
      gi--;
    }
  }
...
int parse_const_primary() {
  ...
  if (p_match(TK_OP, "(")) {
    ...
    int is_cast = 0;
    if (tok_kind[cur_pos] == TK_KW && is_type_keyword(tok_val[cur_pos])) { is_cast = 1; }
    if (tok_kind[cur_pos] == TK_ID && has_typedef(tok_val[cur_pos])) { is_cast = 1; }
    // Also detect (identifier *) pattern as a cast (struct pointer)
    if (tok_kind[cur_pos] == TK_ID && tok_kind[cur_pos + 1] == TK_OP &&
      ...
      int lk = cur_pos + 1;
      while (lk < ntokens && my_strcmp(tok_val[lk], "*") == 0) { lk++; }
      if (lk < ntokens && my_strcmp(tok_val[lk], ")") == 0) { is_cast = 1; }
    }
    if (is_cast) {
      int cd = 1;
      while (cd > 0) {
...
int parse_const_rel() {
  ...
  while (p_match(TK_OP, "<") || p_match(TK_OP, ">") || p_match(TK_OP, "<=") || p_match(TK_OP, ">=")) {
    int *op = tok_val[cur_pos]; p_eat(TK_OP, 0); int rhs = parse_const_shift();
...
int parse_const_eq() {
  ...
  while (p_match(TK_OP, "==") || p_match(TK_OP, "!=")) {
    int *op = tok_val[cur_pos]; p_eat(TK_OP, 0); int rhs = parse_const_rel();
...
int skip_extern_decl() {
  ...
  if (p_match(TK_KW, "struct") || p_match(TK_KW, "union")) {
    cur_pos++;
    if (tok_kind[cur_pos] == TK_ID && !p_match(TK_OP, "{")) {
      ext_stype = my_strdup(tok_val[cur_pos]);
      cur_pos++;
    }
  } else if (tok_kind[cur_pos] == TK_ID && has_typedef(tok_val[cur_pos])) {
    ext_stype = find_typedef(tok_val[cur_pos]);
    if (ext_stype != 0) { ext_stype = my_strdup(ext_stype); }
    cur_pos++;
  } else {
    // Skip base type keywords
    while (cur_pos < ntokens && tok_kind[cur_pos] == TK_KW) { cur_pos++; }
  }
  ...
  if (tok_kind[cur_pos] == TK_ID && !p_match(TK_OP, "(")) {
    int *ext_name = tok_val[cur_pos];
    // Don't register if next token is '(' (that's a function prototype)
    if (cur_pos + 1 < ntokens && my_strcmp(tok_val[cur_pos + 1], "(") != 0) {
      glv_name[nglv] = my_strdup(ext_name);
      if (ext_stype != 0) { glv_stype[nglv] = my_strdup(ext_stype); } else { glv_stype[nglv] = 0; }
      glv_isptr[nglv] = 1;
      glv_arrsize[nglv] = 0 - 1;
      glv_is_char[nglv] = 0;
      nglv++;
    }
  }
...
int parse_typedef(struct SDef **structs, int *ns_ptr) {
  ...
  if (p_match(TK_KW, "struct") || p_match(TK_KW, "union")) {
    ...
    if (p_match(TK_OP, "{")) {
      ...
      if (tag_name != 0) {
        ...
        sd->word_indices = 0;
        sd->nwords = 0;
        sd->field_is_array = my_malloc(td_nf * 8);
    ...
    if (p_match(TK_ID, 0)) {
      ...
      if (tag_name == 0 && td_nf > 0) {
        tag_name = alias;
        // Register struct def with alias name
        sdi = my_malloc(32);
        sdi->name = my_strdup(alias);
        sdi->flds = td_finfo;
        sdi->nflds = td_nf;
        sdi->nwords = 0;
        p_sdefs[np_sdefs] = sdi;
        np_sdefs++;
        ftypes = my_malloc(512 * 8);
        fti = 0;
        while (fti < td_nf) {
          if (td_finfo[fti]->stype != 0 && td_finfo[fti]->is_ptr == 0) {
            ftypes[fti] = my_strdup(td_finfo[fti]->stype);
        ...
        sd = my_malloc(104);
        sd->name = my_strdup(alias);
        sd->fields = td_fields;
        sd->field_types = ftypes;
        sd->nfields = td_nf;
        sd->is_union = is_union_td;
        sd->bit_widths = 0; sd->bit_offsets = 0; sd->word_indices = 0; sd->nwords = 0;
        sd->field_is_array = my_malloc(td_nf * 8);
        sd->field_is_char = my_malloc(td_nf * 8);
        sd->field_is_ptr = my_malloc(td_nf * 8);
        tdi = 0;
  ...
  if (p_match(TK_OP, "(") && tok_kind[cur_pos + 1] == TK_OP && my_strcmp(tok_val[cur_pos + 1], "*") == 0) {
    ...
    if (ptd_is_char) { td_is_char[ntd - 1] = 1; td_is_unsigned[ntd - 1] = ptd_is_unsigned; }
...
struct Program *parse_program() {
  ...
  add_typedef("uint8_t", 0); td_is_char[ntd - 1] = 1; td_is_unsigned[ntd - 1] = 1;
  add_typedef("uint16_t", 0);
  add_typedef("uint32_t", 0);
  add_typedef("uint64_t", 0);
  add_typedef("int8_t", 0); td_is_char[ntd - 1] = 1; td_is_unsigned[ntd - 1] = 0;
  ...
  int ns = 0;
  struct FuncDef **funcs = my_malloc(16384 * 8);
  int nf = 0;
  int **proto_names = my_malloc(16384 * 8);
  int *proto_rip = my_malloc(16384 * 8);
  int *proto_riu = my_malloc(16384 * 8);
  int *proto_is_var = my_malloc(16384 * 8);
  int *proto_np = my_malloc(16384 * 8);
  int **proto_rs = my_malloc(16384 * 8);
  int *proto_rif = my_malloc(16384 * 8);
  int nprotos = 0;
  struct GDecl **globals = my_malloc(16384 * 8);
  int ng = 0;
  struct FuncDef *fd = 0;
  int saved = 0;
  ...
  while (!p_match(TK_EOF, 0)) {
    ...
    if (p_match(TK_KW, "struct") || p_match(TK_KW, "union")) {
      ...
      if (p_match(TK_OP, "{")) {
        ...
        if (p_match(TK_OP, "*") || p_match(TK_ID, 0)) {
          ...
          while (p_match(TK_OP, ",")) {
            ...
            glv_name[nglv] = my_strdup(sv_name2);
            glv_stype[nglv] = my_strdup(sv_stype);
            glv_isptr[nglv] = sv_ptr2;
            glv_arrsize[nglv] = 0 - 1;
            glv_is_char[nglv] = 0;
            nglv++;
          }
          ...
          globals[ng] = sv_gd; ng++;
          glv_name[nglv] = my_strdup(sv_name);
          glv_stype[nglv] = my_strdup(sv_stype);
          glv_isptr[nglv] = sv_ptr;
          glv_arrsize[nglv] = sv_arr;
          glv_is_char[nglv] = 0;
          nglv++;
        } else if (p_match(TK_OP, ";")) {
          p_eat(TK_OP, ";");
      ...
      if (p_match(TK_OP, "{")) {
        ...
        if (p_match(TK_OP, "*") || (tok_kind[cur_pos] == TK_ID && !has_typedef(tok_val[cur_pos]) && tok_kind[cur_pos] != TK_KW
            && !(cur_pos + 1 < ntokens && my_strcmp(tok_val[cur_pos + 1], "(") == 0))) {
      ...
      // Patterns: ((void)0);  funcname(args);  0;  etc.
      {
        ...
        if (p_match(TK_OP, "(")) {
          int pp = cur_pos;
          while (pp < ntokens && my_strcmp(tok_val[pp], "(") == 0) { pp++; }
          if (pp < ntokens && tok_kind[pp] == TK_KW && my_strcmp(tok_val[pp], "void") == 0 &&
              pp + 1 < ntokens && my_strcmp(tok_val[pp + 1], ")") == 0) {
            is_toplevel_expr = 1;
          }
        }
        ...
        if (tok_kind[cur_pos] == TK_ID && !has_typedef(tok_val[cur_pos]) &&
          ...
          while (pd2 > 0 && pp2 < ntokens) {
            if (my_strcmp(tok_val[pp2], "(") == 0) { pd2++; }
            else if (my_strcmp(tok_val[pp2], ")") == 0) { pd2--; }
            pp2++;
          }
          // After ), check for { (direct body) or type keywords (K&R param decls before {)
          int is_knr_func = 0;
          if (pp2 < ntokens && my_strcmp(tok_val[pp2], "{") == 0) { is_knr_func = 1; }
          // K&R param declarations: f(a, b) int a; char *b; { ... }
          if (pp2 < ntokens && tok_kind[pp2] == TK_KW) { is_knr_func = 1; }
          if (!is_knr_func) {
            is_toplevel_expr = 1;
          }
        }
        // bare number at top level (e.g. macro expanding to 0;)
        if (tok_kind[cur_pos] == TK_NUM) {
          is_toplevel_expr = 1;
        }
        if (is_toplevel_expr) {
          while (!p_match(TK_OP, ";") && !p_match(TK_EOF, 0)) { cur_pos++; }
  ...
  while (i < ncg_s) {
    if (my_strcmp(cg_sname[i], sname) == 0) {
      ...
      while (j < cg_snfields[i]) {
        ...
        // But char arrays are stored as a single pointer (1 slot)
        if (cg_s_fa[i] != 0 && cg_s_fa[i][j] > 0) {
          if (cg_s_fc[i] != 0 && cg_s_fc[i][j]) {
            f_slots = 1;
          } else {
    ...
    if (off < 0) {
      ...
      // Check if it's an extern variable (registered in glv table)
      {
        ...
        for (int evi = 0; evi < nglv; evi++) {
          if (my_strcmp(glv_name[evi], e->sval) == 0) { ev_found = 1; break; }
        }
        if (ev_found) {
...
int codegen(struct Program *prog) {
  ...
  for (int gi = 0; gi < prog->nglobals; gi++) {
    ...
    cg_g_ptr_stype[ncg_g] = (gd->is_ptr == 1) ? gd->stype : 0;
  ...
  if (nsl > 0) {
    ...
    while (i < nsl) {
      ...
      emit_s(sl_labels[i]);
      emit_line(":");
      if (sl_has_init[i] == 2 && sl_init_list[i] != 0) {
    ...
    if (si < srclen && __read_byte(src, si) == '#') {
      ...
      // Check for "define"
        ...
        if (si < srclen && __read_byte(src, si) == '(') {
          ...
          // Strip block comments from function-like macro body
          { int *mb_tmp = my_malloc(bend - bstart + 1);
            int mb_ri = bstart; int mb_wi = 0;
            while (mb_ri < bend) {
              if (mb_ri + 1 < bend && __read_byte(src, mb_ri) == '/' && __read_byte(src, mb_ri + 1) == '*') {
                mb_ri = mb_ri + 2;
                int mb_found_end = 0;
                while (mb_ri + 1 < bend) {
                  if (__read_byte(src, mb_ri) == '*' && __read_byte(src, mb_ri + 1) == '/') { mb_ri = mb_ri + 2; mb_found_end = 1; break; }
                  mb_ri++;
                }
                if (mb_found_end == 0) { mb_ri = bend; }
              } else {
                __write_byte(mb_tmp, mb_wi, __read_byte(src, mb_ri));
                mb_wi++; mb_ri++;
              }
            }
            while (mb_wi > 0 && (__read_byte(mb_tmp, mb_wi - 1) == ' ' || __read_byte(mb_tmp, mb_wi - 1) == '\t')) { mb_wi--; }
            __write_byte(mb_tmp, mb_wi, 0);
          ...
          // Strip block comments /* ... */ from macro value
          { int *mv_tmp = my_malloc(vend - vstart + 1);
            int mv_ri = vstart; int mv_wi = 0;
            while (mv_ri < vend) {
              if (mv_ri + 1 < vend && __read_byte(src, mv_ri) == '/' && __read_byte(src, mv_ri + 1) == '*') {
                mv_ri = mv_ri + 2;
                int mv_found_end = 0;
                while (mv_ri + 1 < vend) {
                  if (__read_byte(src, mv_ri) == '*' && __read_byte(src, mv_ri + 1) == '/') { mv_ri = mv_ri + 2; mv_found_end = 1; break; }
                  mv_ri++;
                }
                if (mv_found_end == 0) { mv_ri = vend; }
              } else {
                __write_byte(mv_tmp, mv_wi, __read_byte(src, mv_ri));
                mv_wi++; mv_ri++;
              }
            }
            // Trim trailing whitespace
            while (mv_wi > 0 && (__read_byte(mv_tmp, mv_wi - 1) == ' ' || __read_byte(mv_tmp, mv_wi - 1) == '\t')) { mv_wi--; }
            __write_byte(mv_tmp, mv_wi, 0);
            macro_names[nmacros] = make_str(src, nstart, nend - nstart);
            macro_values[nmacros] = mv_tmp;
          }
...
int main(int argc, int *argv) {
  ...
  { int mi = 0; while (mi < nmacros) {
    macro_nlens[mi] = my_strlen(macro_names[mi]);
    macro_ht_next[mi] = 0 - 1;
    int h = 0; int ci = 0;
    while (ci < macro_nlens[mi]) { h = h * 31 + __read_byte(macro_names[mi], ci); ci++; }
    h = h & 65535;
    macro_ht_next[mi] = macro_ht_head[h];
    macro_ht_head[h] = mi;
    mi++;
  } }
  ...
  { // Always run expansion pass (for __LINE__/__FILE__ even with nmacros==0)
    ...
    { int mti = 0; while (mti < nmacros) {
      if (macro_bodies[mti]) { macro_total = macro_total + my_strlen(macro_bodies[mti]); }
      if (macro_values[mti]) { macro_total = macro_total + my_strlen(macro_values[mti]); }
      mti++;
    } }
          ...
          if (macro_nlens[mc] == ilen &&
            ...
            if (macro_nparams[mc] >= 0) {
              ...
              if (fj < co && __read_byte(cleaned, fj) == '(') {
                ...
                int argcount_ok = 1;
                if (fnargs != macro_nparams[mc] && !(macro_is_variadic[mc] && fnargs >= macro_nparams[mc])) {
                ...
                int *mbody = macro_bodies[mc >= 0 ? mc : 0];
                ...
                while (bi < mblen) {
                  if (is_alpha(__read_byte(mbody, bi))) {
                    ...
                    while (px1 < macro_nparams[mc] && skip_param_sub == 0) {
                    ...
                    if (pf1 == 0 && macro_is_variadic[mc] && my_strcmp(pn1, "__VA_ARGS__") == 0) {
                      // Substitute variadic args
                      int vai = macro_nparams[mc];
                      while (vai < fnargs) {
                        if (vai > macro_nparams[mc]) { __write_byte(expanded, eo, ','); eo++; __write_byte(expanded, eo, ' '); eo++; }
                    ...
                    if (pf1 == 0) {
                      ...
                      if (pl1 == macro_nlens[mc] && my_strcmp(pn1, macro_names[mc]) == 0) {
                    ...
                    int spf = 0;
                    int spi = 0;
                    while (spi < macro_nparams[mc]) {
                      if (my_strcmp(macro_params[mc][spi], spn) == 0) {
                        __write_byte(expanded, eo, '"'); eo++;
          ...
          if (macro_nlens[mc] == ilen &&
            ...
            if (macro_nparams[mc] >= 0) {
              ...
              if (fj2 < co && __read_byte(cleaned, fj2) == '(') {
                ...
                int argcount_ok2 = 1;
                if (fnargs2 != macro_nparams[mc] && !(macro_is_variadic[mc] && fnargs2 >= macro_nparams[mc])) {
                ...
                int *mbody2 = macro_bodies[mc >= 0 ? mc : 0];
                ...
                while (bi2 < mblen2) {
                  if (is_alpha(__read_byte(mbody2, bi2))) {
                    ...
                    while (pi2 < macro_nparams[mc] && skip_psub2 == 0) {
                    ...
                    if (pfound2 == 0 && macro_is_variadic[mc] && my_strcmp(pname2, "__VA_ARGS__") == 0) {
                      int vai2 = macro_nparams[mc];
                      while (vai2 < fnargs2) {
                        if (vai2 > macro_nparams[mc]) { __write_byte(expanded2, eo, ','); eo++; __write_byte(expanded2, eo, ' '); eo++; }
                    ...
                    if (pfound2 == 0) {
                      ...
                      if (plen2 == macro_nlens[mc] && my_strcmp(pname2, macro_names[mc]) == 0) {
                    ...
                    int spf2 = 0;
                    int spi2 = 0;
                    while (spi2 < macro_nparams[mc]) {
                      if (my_strcmp(macro_params[mc][spi2], spn2) == 0) {
                        __write_byte(expanded2, eo, '"'); eo++;
              ...
              int selfref = 0;
              { int sr_vl = my_strlen(macro_values[mc]);
                int sr_nl = macro_nlens[mc];
                int sr_i = 0;
                while (sr_i + sr_nl <= sr_vl) {
                  if (sr_i > 0 && is_alnum(__read_byte(macro_values[mc], sr_i - 1))) { sr_i++; }
                  else {
                    int sr_m = 1;
                    int sr_j = 0;
                    while (sr_j < sr_nl) {
                      if (__read_byte(macro_values[mc], sr_i + sr_j) != __read_byte(macro_names[mc], sr_j)) { sr_m = 0; sr_j = sr_nl; }
                      sr_j++;
                    }
                    if (sr_m && (sr_i + sr_nl >= sr_vl || is_alnum(__read_byte(macro_values[mc], sr_i + sr_nl)) == 0)) { selfref = 1; sr_i = sr_vl; }
                    else { sr_i++; }
                  }
                }
              }
